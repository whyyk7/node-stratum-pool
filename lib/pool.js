...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function GetCoinAddress(finishedCallback){
        if(isNaN(options.address)){
            _this.daemon.cmd('getaccountaddress',[''],function(result){
                    if (result.error){
                        emitErrorLog('getCoinAddress call failed for daemon instance ' +
                            result.instance.index + ' with error ' + JSON.stringify(result.error));
                        return;
                    }else{
                        options.address = result.response;
                        finishedCallback();
                    }
                }, true
            );
    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            ...
            console.log(rpcResults.validateaddress);
